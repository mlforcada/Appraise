...
def draw_sentences(src, ref, mt, tag, morph, n):
    ...
    tag and morph: ref put through Apertium morphological analyzer
...
def everything(*args, **options):
    ...
    MORPH PATH - path to Apertium morphological analyzer mode, e.g. /foo/apertium-eo-en/modes/en-eo-morph.mode.
    Note that in this case target language code comes first. For Esperanto to English that would be en-eo-morph.mode.
    This mode may need to be created first depending on a language pair. For some pairs, this mode is called 'anmor'.
    ...
    tagger = options['morph_path']

    # generate texts
    if mt_mode.endswith('.mode'):
    	mt = call_apertium(src, mt_mode)
        tag = call_apertium(ref, tagger)
	morph = tag
    elif mt_mode.endswith('.txt'):
        mt = open('mt_path').read()
    elif mt_mode.endswith('.lst'):
        f = open(value)
